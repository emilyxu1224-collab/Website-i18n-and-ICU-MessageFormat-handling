...
let I18nDemo = "I18nDemo.js";//$NON-NLS-L$
...
let dummyOrders = () => {
    ...
    //the item counts are deliberately 1 and 3 so the order list shows both
    //the singular and the plural branch of the same message
    let order2 = new Order(900, new Date(fiveDays), null, 1); //$NON-NLS-L$
    let order1 = new Order(68500, new Date(twoDays), null, 3); //$NON-NLS-L$
...
if(localStorage.getItem("orderHistory") !== null) {
    ...
    for(let order of orders) {
        //order[3] is the item count; older saved orders do not have it, so default to 1
        let orderObj = new Order(parseInt(order[2]), new Date(order[0]), parseInt(order[1]), parseInt(order[3]) || 1); //$NON-NLS-L$
...
const supportedLocales = ["en-US", "de-DE", "zh-CN", "en-XA"];
if(supportedLocales.indexOf(savedLocale) === -1) {
