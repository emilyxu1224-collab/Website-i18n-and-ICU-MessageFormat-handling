...
let Checkout = {
    ...
    render: async () => {
        ...
        if (Object.keys(shoppingCart).length === 0 && shoppingCart.constructor === Object) {
            ...
            //"3 items, total $12,300" — the comma, the digit grouping and the
            //currency position all come from the locale, not from this file
            let orderSummary = i18n.t("Checkout", "orderSummary", {
                count: itemTotal,
                total: i18n.currencyText(total)
            });
            ...
            <div class="cartTotal">
                ...
                <p class="summaryLine">${orderSummary}</p>
                <p class="summaryLine" id="deliveryEstimate">${estimateFor("lightspeed")}</p>
            </div>
    ...
    , after_render: async () => {
        ...
        //update the delivery sentence when the shipping method changes.
        //The whole sentence is re-selected from the message, not patched, so
        //languages that put the date first still read correctly.
        var methodSelect = document.querySelector("#shipMethod");
        var estimateLine = document.querySelector("#deliveryEstimate");
        if (methodSelect && estimateLine) {
            methodSelect.addEventListener("input", () => {
                estimateLine.textContent = estimateFor(methodSelect.value);
            }, false);
        }
...
/**
 * Builds the delivery sentence for a shipping method.
 * The method name is a KEY ("lightspeed"), never a translated word, so the
 * select branch keeps matching in every language.
 */
var estimateFor = (method) => {
    const daysByMethod = { lightspeed: 0, overnight: 1, standard: 5 };
    const days = daysByMethod[method] === undefined ? 5 : daysByMethod[method];
    const arrival = new Date(Date.now() + days * 24 * 60 * 60 * 1000);
    return i18n.t("Checkout", "estimatedDelivery", { method: method, date: arrival });
}

//handle order placement
var placeOrder = () => {
    let order = new Order(total, new Date(), null, itemTotal); //$NON-NLS-L$
    saveOrder(order);
...
let saveOrder = (newOrder) => {
    let orders = [];
    let orderString = [newOrder.orderDate.toString(), newOrder.orderNumber.toString(), newOrder.total.toString(), newOrder.itemCount.toString()]; //$NON-NLS-L$
