...
let Cart = {
    render: async () => {
        ...
        //count the items so the header can say "1 item" / "3 items" / "购物车为空"
        let itemTotal = 0;
        for (let key in shoppingCart) {
            itemTotal += shoppingCart[key].qty;
        }
        //ONE message decides the plural form; the code never picks "s" itself
        let itemCount = i18n.t("Cart", "itemCount", { count: itemTotal });
